...
cv2.createTrackbar('guassian_std','frame',25, 50, nothing)
cv2.createTrackbar('sp_prob','frame',25, 100, nothing)
prob, std = 100, 100
#last = np.zeros(2)
...
while(True):
    ...
    gray = sp_noise(gray, prob/1000)
    gray = guassian_noise(gray, std = std/100)
    ...
    std = cv2.getTrackbarPos('guassian_std','frame')
    prob = cv2.getTrackbarPos('sp_prob','frame')
    _, output = cv2.threshold(gray, thres, 255, cv2.THRESH_BINARY_INV)
    #binary_blur = cv2.medianBlur(output, 11)
    #output = binary_blur
